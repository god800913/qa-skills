...
### 1. 후보 풀 추출 (스크립트, 결정론적)
```bash
uv run scripts/inspect_master.py <xlsx>                                   # 탭 목록
uv run scripts/select_automation_candidates.py <xlsx> --tab "<탭>"        # 분류된 JSON
```
JSON 결과 그룹: `candidates` / `excluded_automated` / `excluded_skip` / `unclassified` / `unknown_check` + `counts`.
- 후보 = `Automation Check`가 All/iOS/Android인 행 중 **`Automation TC_ID`가 비어 있는 행만**.
- `Skip` 행과 이미 자동화된 행(`Automation TC_ID` 존재)은 후보에서 제외.
- `Automation Check`가 비어 있는 행은 `unclassified`(분류 미정), 그 외 비표준 값은 `unknown_check` — 임의 재분류 금지, 리포트에 그대로 표기.
