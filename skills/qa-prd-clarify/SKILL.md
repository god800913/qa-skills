...
### 1. PRD 수집
- Notion URL이면 `reference/notion-fetch-policy.md`에 따라 fetch: 도구 런타임 감지, 페이지 + 자식 블록 + **내부 링크 1-depth follow** + 인라인 DB. PDF·이미지 한계와 스크린샷 입력 처리도 같은 문서 참조.
- 본문이 50단어 미만이면 "PRD가 비어있는 것 같아요. 본문을 붙여 주세요" 안내, 중단.
- 임베드된 Figma 링크/이미지가 있으면 함께 컨텍스트로 활용.
- 리포트 머리에 **소스 매니페스트** (포함/제외 소스) 필수 — 정책 문서 §4.
