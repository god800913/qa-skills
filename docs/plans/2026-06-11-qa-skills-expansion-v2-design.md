...
| 항목 | 결정 |
|---|---|
| Result 컬럼 enum | `Pass / Fail / Block / N/T / N/A` 고정. enum 외 값은 `unknown`으로 분류하고 리포트에 경고. **소유 스크립트는 `parse_results.py`** — `validate_format.py`는 확장하지 않는다 (format-rules.md에 추가 시 parse_results 스코프임을 명시) |
...
| `qa-minimal-coverage` 서식 | 원본 셀 값만 보존(plain value), 원본 TC_ID 유지 + 별도 실행 순서 컬럼 추가. **원본 탭의 전체 컬럼을 범용 보존** — Mutual 탭(`A`/`B` 컬럼, `Test Reproduce`)도 깨지지 않게 |
...
- **MCP 도구 표면**: 특정 서버를 하드코딩하지 않고 "연결된 Figma MCP를 감지해 사용"으로 작성한다. `reference/figma-usage.md`에 공식 Dev Mode MCP와 framelink 두 계열의 연결 방법·대표 도구명을 모두 짧게 안내.
- **PRD 스냅샷 규약을 이 라운드에서 함께 추가한다** — 같은 SKILL.md를 Round 2에서 또 수정하지 않도록. (`qa-prd-clarify`의 스냅샷 단계만 Round 2에서 수정)
- **변경 파일**: `skills/qa-generate-tc/SKILL.md` 워크플로우 확장(Figma + 스냅샷), `reference/figma-usage.md` 신규. 결정론 스크립트 변경 없음.
...
- **전제 규약**: "PRD 읽은 직후 스냅샷 저장" 단계 — `qa-generate-tc`는 Round 0에서 이미 반영, `qa-prd-clarify`는 이 라운드에서 수정
...
| 신규 | 용도 | 사용 스킬 | 작업 라운드 |
|---|---|---|---|
| `shared/summary_xlsx.py` | 시트 스펙 리스트(`[{title, headers, rows}]`) → 요약 워크북 생성 (범용) | risk-analysis, regression-scope | Round 1 |
| `shared/select_minimal_coverage.py` | TC 후보 스코어링·선정 | minimal-coverage | Round 1 |
| `shared/export_minimal_coverage.py` | 5시트 워크북 생성 (원본 전체 컬럼 범용 보존) | minimal-coverage | Round 1 |
| `shared-reference/risk-taxonomy.md` | 리스크 분류 체계 (핵심 플로우, 결제, 매치·콜·실시간, 신고·차단, 인증·권한, Remote Config·실험, 크로스플랫폼, 마이그레이션) | risk-analysis, regression-scope, minimal-coverage, exploratory-charter | Round 1 |
| `shared/parse_results.py` | Result enum 파싱·탭/섹션/Priority 집계 | test-result-report, automation-candidates | Round 2 |
| `shared-reference/format-rules.md` 갱신 | Result enum 5값 추가 — `parse_results.py` 스코프임을 명시 (validate_format 검출 항목 아님) | test-result-report | Round 2 |
| `tests/fixtures/sample_tc_executed.xlsx` + `scripts/make_executed_fixture.py` | Result 값이 채워진 실행 완료 픽스처 | parse_results 테스트 | Round 2 |

기존 규약 유지: 스킬 구조(`SKILL.md` + `reference/` + `examples/` + `scripts/`), `scripts/sync_shared.py` 동기화, 원본 xlsx 절대 수정 금지. 새 스킬 번들이 shared 파일을 쓰려면 **동일 이름 placeholder 파일을 번들에 먼저 만들어야** sync 대상이 된다 (sync_shared 동작 방식) — 각 라운드 체크리스트에 포함할 것.

## 테스트·검증 전략

- **결정론 스크립트** (summary_xlsx, parse_results, select/export_minimal_coverage): pytest TDD (RED → GREEN), 기존 테스트 스위트(현재 46개)에 누적.
