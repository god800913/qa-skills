...
- **입력**: PRD (필수), Figma 링크/임베드 (옵션), append 시 마스터 xlsx + 타겟 탭, `qa-prd-clarify`·`qa-prd-diff` 리포트 (옵션 — Blocker·Open Questions는 Expected Result `TBD (PM 확인 필요)` + `(Blocker)` Comment로 TC에 반영)
...
├── tests/                  # 단위 테스트 123개 — shared/ + sync 정책 + 스킬 메타
...
```bash
uv run pytest          # 123개 — shared/ 스크립트 전부 + sync_shared 정책 + 스킬 번들 메타(frontmatter·참조 실존·README 목록 일치)
```
