...
- **입력**: PRD (필수), Figma 링크/임베드 (옵션), append 시 마스터 xlsx + 타겟 탭
- **처리**: `inspect_master.py`로 타겟 탭 메타(컬럼·섹션·템플릿 타입·샘플 행) 학습 → 휴리스틱(`prioritization-guide.md`)으로 P1~P4·OS·Automation·mutual 분기 → **사용자 컨펌 루프**(xlsx 만들기 전 md 미리보기) → `new_workbook.py` 또는 `append_to_master.py`
...
- **Result enum (팀 고정)**: `Pass / Fail / Block / N/T / N/A` (대소문자 무시·trim 매칭, 빈 셀 = 미입력). 이외 값은 `unknown`으로 집계되어 리포트에 경고. `qa-test-result-report`만 이 컬럼을 검사한다 (작성 시점엔 비어 있는 게 정상이므로 `qa-review-tc`는 미검사).
- **PRD 스냅샷 규약**: PRD를 fetch한 스킬(`prd-clarify`·`generate-tc`·`prd-diff`)은 본문을 `./prd-snapshots/<기능명>-<YYYYMMDD>.md`로 저장한다. Notion이 버전 히스토리 MCP를 안 주므로, 이 스냅샷이 `qa-prd-diff`의 "과거 버전" 대체물이다.
- **원본 불변 (immutability)**: xlsx를 쓰는 스킬은 원본 마스터를 절대 수정하지 않고 항상 새 파일로 출력한다 (collision 시 `(2)`·`(3)` 접미사). `Result`·`Jira no.`·`Automation TC_ID` 같은 사람-소관 컬럼은 스킬이 채우지 않고 안내만 한다.
...
│   ├── reference/          #   체크리스트·룰·용어집 (shared-reference에서 동기화)
│   ├── scripts/            #   결정론 스크립트 (shared에서 동기화)
│   └── examples/           #   출력 예시
...
- 설계·라운드별 구현 기록은 `docs/plans/`. (R0~R2 출시 완료, R3 = exploratory·automation·bug-report)
